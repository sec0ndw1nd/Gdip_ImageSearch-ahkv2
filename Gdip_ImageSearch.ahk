;**********************************************************************************
;
; Gdip_ImageSearch For AHK v2.0.2 and above
; Requires AHKv2-Gdip(https://github.com/buliasz/AHKv2-Gdip/tree/master) @buliasz
; Follows the original license
;     Licensed under CC BY-SA 3.0 -> http://creativecommons.org/licenses/by-sa/3.0/
;
; ---- Original source info
; author: MasterFocus
; source: https://github.com/MasterFocus/AutoHotkey/blob/master/Functions/Gdip_ImageSearch/Gdip_ImageSearch.ahk
;
; ---- Changes Log
; ByRef var -> &var
; If var between Lower and Upper -> If var >= Lower and var <= Upper
; Use IsSet(var) to avoid warn "This Variable has not been assigned a value"
; VarSetCapacity() -> Buffer() or VarSetStrCapacity()
;
;**********************************************************************************
...
Gdip_ImageSearch(pBitmapHaystack, pBitmapNeedle, &OutputList := ""
    , OuterX1 := 0, OuterY1 := 0, OuterX2 := 0, OuterY2 := 0, Variation := 0, Trans := -1
    ...
    DumpCurrentNeedle := false
    If Trans >= 0 and Trans <= 0xFFFFFF
    {
...
Gdip_SetBitmapTransColor(pBitmap, TransColor) {
    ...
    ;   ; MCode_SetBmpTrans = "{x86},{x64}"
        ...
        LEN := StrLen(MCode_SetBmpTrans) // 2
        _SetBmpTrans := Buffer(LEN, 0)
        Loop LEN {
            NumPut("UChar", "0x" . SubStr(MCode_SetBmpTrans, (2 * A_Index) - 1, 2), _SetBmpTrans, A_Index - 1)
        }
        MCode_SetBmpTrans := ""
        DllCall("VirtualProtect", Ptr, _SetBmpTrans.Ptr, Ptr, _SetBmpTrans, "UInt", 0x40, PtrA, 0)
    }
    If !pBitmap
        Return -2001
    ...
    */
    ...
    Gdip_FromARGB(TransColor, &A, &R, &G, &B), VarSetStrCapacity(&TransColor, 0), TransColor := Buffer(3, 255)
    NumPut("UChar", B, TransColor, 0), NumPut("UChar", G, TransColor, 1), NumPut("UChar", R, TransColor, 2)
    MCount := 0
    E := DllCall(_SetBmpTrans.Ptr, Ptr, Scan, "int", W, "int", H, "int", Stride, Ptr, &TransColor, "int*", MCount, "cdecl int")
    ...
    , SearchDirection := 1, Instances := 0, LineDelim := "`n", CoordDelim := ",")
{
    ...
    */
        ...
        , nScan, nWidth, nHeight, &FoundX, &FoundY, OuterX1, OuterY1, OuterX2, OuterY2, Variation, SearchDirection)))
    {
        ...
        Outer%P%%iPre% := Found%P%+step%P%
        Inner%N%%iNon% := Found%N%+step%N%
        Inner%P%1 := Found%P%
        Inner%P%2 := Found%P%+1
        While (!(OutputCount == Instances) && (0 == Gdip_LockedBitsSearch(hStride, hScan, hWidth, hHeight, nStride
    ...
    , &x := "", &y := "", sx1 := 0, sy1 := 0, sx2 := 0, sy2 := 0, Variation := 0, sd := 1)
{
    ...
    ;   ; MCode_ImageSearch = "{x86},{x64}"
        ...
        LEN := StrLen(MCode_ImageSearch) // 2
        _ImageSearch := Buffer(LEN)
    ...
    x := 0
    y := 0
    E := DllCall(_ImageSearch.Ptr, "int*", &x, "int*", &y, Ptr, hScan, Ptr, nScan, "int", nWidth, "int", nHeight
